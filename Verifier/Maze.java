...
public class Maze {
	...
	protected void simulate(int score) {
		...
		// Starts at two based on the way the game on Kongregate works. Squeaky needs to
		// move onto the start vertex
		// and move off of the finish vertex, simply starting at 2 simulates these
		// steps.
		int steps = 2;
	...
	private ArrayList<ArrayList<Integer>> parseFileCreateMaze(File mazeFile) {
		...
		try {
			...
			System.out.println(
					"File not found, please specify the filename (INCLUDING EXTENSION) in the directory you are running the verifier from.");
