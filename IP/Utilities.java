...
public class Utilities {
	...
	public ArrayList<ArrayList<GRBVar>> createGrid(int obs, String modelType) {
			...
			for (int column = 0; column < size; column++) {
				try { 
					if ((row == 0 && column == 0) || (row == size - 1 && column == 0)) {
						currentVar = GRBModel.addVar(0.0, 0.0, 0.0, GRB.BINARY,
								"grid_row_is_" + row + "_column_is_" + column);
					} else {
		...
		try {
			GRBModel.addConstr(setNumberOfObstacles, GRB.LESS_EQUAL, (size * size) - size - 1,
					"numobsmustbelessthan" + ((size * size) - size - 1));
		} catch (
		GRBException e) {
			e.printStackTrace();
		}
		...
		try {
			if (settingStartMove(row, column, timestep))
				currentVar = GRBModel.addVar(1.0, 1.0, 0.0, GRB.BINARY,
						"visits_row_is_" + row + "_column_is_" + column + "_timestep_is_" + timestep);
			else
				currentVar = GRBModel.addVar(0.0, timestep + 2, 0.0, GRB.INTEGER,
						"visits_row_is_" + row + "_column_is_" + column + "_timestep_is_" + timestep); //
		} catch (GRBException e) {
			e.printStackTrace();
		}
			...
			for (int column = 0; column < size; column++) {
				...
				for (int timestep = 0; timestep < upperBound; timestep++) {
					try {
						if (settingStartMove(row, column, timestep))
							currentVar = GRBModel.addVar(1.0, 1.0, 0.0, GRB.BINARY,
									"decisions_row_is_" + row + "_column_is_" + column + "_timestep_is_" + timestep);
						else
							currentVar = GRBModel.addVar(0.0, 1.0, 0.0, GRB.BINARY,
									"decisions_row_is_" + row + "_column_is_" + column + "_timestep_is_" + timestep);
					} catch (GRBException e) {
	...
	public void printToFile(ArrayList<ArrayList<ArrayList<GRBVar>>> visits,
		ArrayList<ArrayList<ArrayList<GRBVar>>> decisions, ArrayList<ArrayList<GRBVar>> grid) {

		for (int timestep = 0; timestep < upperBound; timestep++) {
			for (int row = 0; row < size; row++) {
				for (int column = 0; column < size; column++) {
					try {
						System.out.print(visits.get(row).get(column).get(timestep).get(GRB.DoubleAttr.X) + " ");
					} catch (GRBException e) {
						e.printStackTrace();
					}
