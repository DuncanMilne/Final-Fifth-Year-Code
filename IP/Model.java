...
public class Model {
	...
	public Model(String modelType, int size, int score, int upperBound, int obs) {
		try {
			GRBEnv = new GRBEnv();
		...
		grid = utilities.createGrid(obs, modelType);
		visits = utilities.createVisits();
		decisions = utilities.createDecisions();
		try {
			addConstraints(modelType, obs);
		} catch (GRBException e) {
			e.printStackTrace();
		}
	...
	public void addConstraints(String modelType, int obs) throws GRBException {
		...
		if (modelType.equals("ObsWithinRange"))
			obsWithinRange();
		if (modelType.equals("LimitRowsAndColumns"))
			limitRowsAndColumns();
		if (modelType.equals("PreciseObs"))
			limitObs(size);
		if (!modelType.equals("TransitiveClosure"))
			orderNSquaredConstraints();
		else 
			transitiveClosureFeasibilityConstraints();
	}

	// argument of -1 means ranged obs
	private void limitObs(int size) {
		GRBLinExpr numberOfObs = new GRBLinExpr();
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				numberOfObs.addTerm(1.0, grid.get(i).get(j));
			}
		}
		
		// lazy implementation is okay since evaluation was not continued past 5x5 with this model
		try {
			if (size == 3)
				GRBModel.addConstr(numberOfObs, GRB.EQUAL, 1, "");
			else if (size == 4)
				GRBModel.addConstr(numberOfObs, GRB.EQUAL, 2, "");
			else if (size == 5)
				GRBModel.addConstr(numberOfObs, GRB.EQUAL, 5, "");
	...
	// Auxiliary Constraint LRAC
	private void limitRowsAndColumns() {
		GRBLinExpr sumOfCurrentIteration;

		for (int i = 0; i < size; i++) {
			sumOfCurrentIteration = new GRBLinExpr();
			for (int j = 0; j < size; j++) {
				sumOfCurrentIteration.addTerm(1.0, grid.get(i).get(j));
			}
			try {
				GRBModel.addConstr(sumOfCurrentIteration, GRB.LESS_EQUAL, size - (size / 3), "");
			} catch (GRBException e) {
				e.printStackTrace();
			}
		}
		...
		for (int i = 0; i < size; i++) {
			sumOfCurrentIteration = new GRBLinExpr();
			for (int j = 0; j < size; j++) {
				sumOfCurrentIteration.addTerm(1.0, grid.get(j).get(i));
			}
			try {
				GRBModel.addConstr(sumOfCurrentIteration, GRB.LESS_EQUAL, size - (size / 3), "");
			} catch (GRBException e) {
	...
	// Auxiliary Constraint OWR
	private void obsWithinRange() {
		GRBLinExpr numberOfObs = new GRBLinExpr();
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				numberOfObs.addTerm(1.0, grid.get(i).get(j));
		...
		try {
			GRBModel.addConstr(numberOfObs, GRB.LESS_EQUAL, ((size * size) / 2), "");
			GRBModel.addConstr(numberOfObs, GRB.GREATER_EQUAL, ((size * size) / 4), "");
		} catch (GRBException e) {
			e.printStackTrace();
		}
	}
	...
	// Count visits at each cell at each time step
	...
	// Stop moving after reaching finish cell
	...
	public void moveDown() throws GRBException {
		...
		GRBLinExpr downLeqRightExpr = new GRBLinExpr(), downLeqLeftExpr = new GRBLinExpr(),
				downLeqUpExpr = new GRBLinExpr();

		// Build expression to count Squeakys visits in each direction
		...
		// Determine whether down has <= visits of all other directions
		GRBModel.addGenConstrIndicator(downLeqRight, 1, downLeqRightExpr, GRB.LESS_EQUAL, 0,
				"1indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(downLeqRight, 0, downLeqRightExpr, GRB.GREATER_EQUAL, 1,
				"2indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(downLeqLeft, 1, downLeqLeftExpr, GRB.LESS_EQUAL, 0,
				"3indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(downLeqLeft, 0, downLeqLeftExpr, GRB.GREATER_EQUAL, 1,
				"4indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(downLeqUp, 1, downLeqUpExpr, GRB.LESS_EQUAL, 0,
				"5indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(downLeqUp, 0, downLeqUpExpr, GRB.GREATER_EQUAL, 1,
				"6indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);

		// If down has less than all others, we move down
		GRBVar[] toSatisfy = new GRBVar[] { downLeqRight, downLeqLeft, downLeqUp,
				decisions.get(row).get(column).get(timestep)};
		...
		GRBModel.addConstr(decisions.get(row + 1).get(column).get(timestep + 1), GRB.GREATER_EQUAL, satisfied,
				"moveDownRow_" + row + "_Column_" + column + "_Timestep_" + timestep);
	}
	...
	public void moveRight() throws GRBException {
		...
		GRBLinExpr rightLessThanDownExpr = new GRBLinExpr(), rightLeqLeftExpr = new GRBLinExpr(),
				rightLeqThanUpExpr = new GRBLinExpr();
		...
		GRBModel.addGenConstrIndicator(rightLessThanDown, 1, rightLessThanDownExpr, GRB.LESS_EQUAL, 0,
				"7indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(rightLessThanDown, 0, rightLessThanDownExpr, GRB.GREATER_EQUAL, 1,
				"8indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(rightLeqLeft, 1, rightLeqLeftExpr, GRB.LESS_EQUAL, 0,
				"9indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(rightLeqLeft, 0, rightLeqLeftExpr, GRB.GREATER_EQUAL, 1,
				"10indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(rightLeqUp, 1, rightLeqThanUpExpr, GRB.LESS_EQUAL, 0,
				"11indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(rightLeqUp, 0, rightLeqThanUpExpr, GRB.GREATER_EQUAL, 1,
				"12indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);

		GRBVar[] toSatisfy = new GRBVar[] { rightLessThanDown, rightLeqLeft, rightLeqUp,
				decisions.get(row).get(column).get(timestep)};
		...
		GRBModel.addConstr(decisions.get(row).get(column + 1).get(timestep + 1), GRB.GREATER_EQUAL, satisfied,
				"moveRightRow_" + row + "_Column_" + column + "_Timestep_" + timestep);
	}

	public void moveLeft() throws GRBException {
		GRBVar leftLessThanDown = standardBoolean(), leftLessThanRight = standardBoolean(),
				leftLeqUp = standardBoolean();

		GRBLinExpr leftLessThanDownExpr = new GRBLinExpr(), leftLessThanRightExpr = new GRBLinExpr(),
				leftLeqUpExpr = new GRBLinExpr();
		...
		GRBModel.addGenConstrIndicator(leftLessThanDown, 1, leftLessThanDownExpr, GRB.LESS_EQUAL, 0,
				"13indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(leftLessThanDown, 0, leftLessThanDownExpr, GRB.GREATER_EQUAL, 1,
				"14indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(leftLessThanRight, 1, leftLessThanRightExpr, GRB.LESS_EQUAL, 0,
				"15indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(leftLessThanRight, 0, leftLessThanRightExpr, GRB.GREATER_EQUAL, 1,
				"16indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(leftLeqUp, 1, leftLeqUpExpr, GRB.LESS_EQUAL, 0,
				"17indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(leftLeqUp, 0, leftLeqUpExpr, GRB.GREATER_EQUAL, 1,
				"18indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);

		GRBVar[] toSatisfy = new GRBVar[] { leftLessThanDown, leftLessThanRight, leftLeqUp,
				decisions.get(row).get(column).get(timestep)};
		...
		GRBModel.addConstr(decisions.get(row).get(column - 1).get(timestep + 1), GRB.GREATER_EQUAL, satisfied,
				"moveLeftRow_" + row + "_Column_" + column + "_Timestep_" + timestep);
	}

	public void moveUp() throws GRBException {
		GRBVar upLessThanDown = standardBoolean(), upLessThanRight = standardBoolean(),
				upLessThanLeft = standardBoolean();

		GRBLinExpr upLessThanDownExpr = new GRBLinExpr(), upLessThanRightExpr = new GRBLinExpr(),
				upLessThanLeftExpr = new GRBLinExpr();
		...
		GRBModel.addGenConstrIndicator(upLessThanDown, 1, upLessThanDownExpr, GRB.LESS_EQUAL, 0,
				"19indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(upLessThanDown, 0, upLessThanDownExpr, GRB.GREATER_EQUAL, 1,
				"20indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(upLessThanRight, 1, upLessThanRightExpr, GRB.LESS_EQUAL, 0,
				"21indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(upLessThanRight, 0, upLessThanRightExpr, GRB.GREATER_EQUAL, 1,
				"22indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(upLessThanLeft, 1, upLessThanLeftExpr, GRB.LESS_EQUAL, 0,
				"23indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);
		GRBModel.addGenConstrIndicator(upLessThanLeft, 0, upLessThanLeftExpr, GRB.GREATER_EQUAL, 1,
				"24indicatorconstraintrow" + row + "column" + column + "timestep" + timestep);

		GRBVar[] toSatisfy = new GRBVar[] { upLessThanDown, upLessThanRight, upLessThanLeft,
				decisions.get(row).get(column).get(timestep)};
	...
	public void obstacleConstraints() {
		...
		try {
			...
			GRBModel.addConstr(obstaclePlusMovement, GRB.LESS_EQUAL, 1,
					"obsplusmovementrow" + row + "column" + column + "timestep" + timestep);
		} catch (GRBException e) {
			e.printStackTrace();
		}
	}
	...
	public void transitiveClosureFeasibilityConstraints() throws GRBException {
		...
		TkMinus1 = new GRBVar[size * size][size * size]; // pathtostart[0][i] (first row) represents the distance from
															// 1,1 to all other vertices. when doing with size = 3, this will be a 9x9
		...
		for (int row = 0; row < size * size; row++) {
			for (int column = 0; column < size * size; column++) {
				if (row != column) {
					TkMinus1[row][column] = GRBModel.addVar(0.0, 1.0, 0.0, GRB.BINARY,
							"FWmatrix row " + row + " column " + column);
					listOfNonAccessedVars.add(TkMinus1[row][column]);

				} else {
					TkMinus1[row][column] = GRBModel.addVar(1.0, 1.0, 0.0, GRB.BINARY,
							"FWmatrix row " + row + " column " + column); // There is a path between i and i
				}
			}
		}
		...
		for (int row = 0; row < size * size; row++) {
			if (row < (size * size) - size) {
				addEdgeDown(TkMinus1[row][row + size], row); // there is an edge between each pair of adjacent cells
		...
		// an edge exists if they are adjacent and neither has an obstacle. if an edge
		// exists, variable is 1.
		for (int k = 0; k < size * size; k++) { // for every vertex, try to use it as an intermediate vertex between
												// every other pair of vertices
			Tk = new GRBVar[size * size][size * size]; // pathtostart[0][i] (first row) represents the distance from 1,1
														// to all other vertices. when doing with size = 3, this will be
														// a 9x9
			for (int i = 0; i < size * size; i++) {
				for (int j = 0; j < size * size; j++) {
					Tk[i][j] = GRBModel.addVar(0.0, 1.0, 0.0, GRB.BINARY, "Tk_row" + "_" + i + "_column_" + j);
					GRBVar Cijk = conjunction(k, i, j);
					GRBModel.addGenConstrOr(Tk[i][j], new GRBVar[] { Cijk, TkMinus1[i][j] },
							"Tk_row" + "_" + i + "_column_" + j);
				}
			}
			TkMinus1 = Tk;
	...
	GRBVar conjunction(int k, int i, int j) throws GRBException {
		GRBVar Cijk = GRBModel.addVar(0.0, 1.0, 0.0, GRB.BINARY,
				"Cijk_row_" + row + "_column_" + column + "_timestep_" + timestep);
		GRBVar tikMinus1 = TkMinus1[i][k];
		GRBVar tkjMinus1 = TkMinus1[k][j];
		GRBModel.addGenConstrAnd(Cijk, new GRBVar[] { tikMinus1, tkjMinus1 },
				"CijkConstraint_row" + row + "_column_" + column + "_timestep_" + timestep);
		return Cijk;
	}
	...
	void addEdgeDown(GRBVar currentVar, int row) throws GRBException {
		...
		edgeExists3.addTerms(new double[] { -1, -1 },
				new GRBVar[] { grid.get(row / size).get(row % size), grid.get(row / size + 1).get(row % size) });
	...
	void addEdgeRight(GRBVar currentVar, int row) throws GRBException {
		...
		edgeExists3.addConstant(1.0);
		edgeExists3.addTerms(new double[] { -1, -1 },
				new GRBVar[] { grid.get(row / size).get(row % size), grid.get(row / size).get(row % size + 1) });
		GRBModel.addConstr(currentVar, GRB.GREATER_EQUAL, edgeExists3, "edgedown");
	}
	...
	public void orderNSquaredConstraints() {
		// first dimension is the row index, second is the column index and third is the
		// timestep (not our usual timestep)
	...
	// At timestep k we sum up all of squeakyDecision variables at k and make sure
	// they are leq than 1
	...
	public void setObjectiveFunction() {
		...
		// Evaluation was not continued with this implementation so this lazy implementation is fine
		try {
			if (size == 3)
				GRBModel.addConstr(objective, GRB.GREATER_EQUAL, 10, "objectivefunction");
			if (size == 4)
				GRBModel.addConstr(objective, GRB.GREATER_EQUAL, 25, "objectivefunction");
			if (size == 5)
				GRBModel.addConstr(objective, GRB.GREATER_EQUAL, 52, "objectivefunction");
