...
public class BFSFeasibilityPreciseObs {
	...
	// Use Breadth First Search to determine whether maze is feasible
	public static boolean feasible() {
		...
		for (int i = 0; i < maze.length; i++) {
			for (int j = 0; j < maze[i].length; j++) {
				allNodes.get(i).add(new Node(i, j, maze[i][j]));
			}
		}
		...
		if (breadthFirstSearch(allNodes.get(0).get(0)))
			return true;

		return false;
	}

	static boolean breadthFirstSearch(Node node) {
		if (visited.contains(node))
			return false;
		if (node.row == n - 1 && node.column == 0)
			return true;
		visited.add(node);
		for (Node child : node.children)
			if (breadthFirstSearch(child))
				return true;
		return false;
	}
