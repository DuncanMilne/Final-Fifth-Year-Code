...
public class HammingDistanceBFSMaze {
ArrayList<Node> visited; 
	ArrayList<ArrayList<Node>> allNodes; private boolean[] maze;
	private Integer[] visits;
	Location<Integer, Integer> location;

	public HammingDistanceBFSMaze(boolean[] maze) {
	}
	...
	public boolean feasible() {
		...
		for (int i = 0; i < 13;i++){
			for (int j= 0; j < 13; j++) {
				if (maze[(i*13)+j] == true)
					allNodes.get(i).add(new Node(i,j, 1));
				else
					allNodes.get(i).add(new Node(i,j, 0));
	...
	boolean breadthFirstSearch(Node node) {
		if (visited.contains(node))
			return false;
		if (node.row==13-1&&node.column==0)
			return true;
		visited.add(node);
